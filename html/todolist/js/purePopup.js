...
;(function() {
    ...
    Popup.prototype.setParams = function (params, callback) {
        ...
        this.params.values = {name: 'Jean-michel'};
        this.params.ids = {name: 'Jarre'};
        if (params) {
    ...
    Popup.prototype.prompt = function (p, c) {
        ...
        var inputsHtml = '', tabIndex = 0;
            for (var i in this.params.inputs) inputsHtml += '<label for="purePopupInputs_'+i+'">'+this.params.inputs[i]+'</label><input id="'+((this.params.ids[i]!=' ')?this.params.ids[i]:"purePopupInputs_"+i)+'" name="'+i+'" type="text" tabindex="'+(tabIndex++)+'" value="'+this.params.values[i]+'">';


        var buttonsHtml = '';
        for (var i in this.params.buttons) buttonsHtml += '<span class="purePopupButton _'+i+'_" onclick="Materialize.toast(\'Sauvegardé!\', 4000, \'rounded\')">'+this.params.buttons[i]+'</span>';
    ...
    Popup.prototype.infoPrompt = function (p, c) {
        this.type = 'prompt';
        this.setParams(p, c);
        
        var inputsHtml = '', tabIndex = 0;
            for (var i in this.params.inputs) inputsHtml += '<label for="purePopupInputs_'+i+'">'+this.params.inputs[i]+this.params.values[i]+'</label>';
